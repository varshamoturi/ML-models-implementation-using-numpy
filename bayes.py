...
def load_docs(docs_dirname:str) -> Sequence[Sequence]:
    ...
    """
    docs = []
    docs_dirname = filelist(docs_dirname)
    for doc_names in docs_dirname:
        if '.txt' in doc_names:
            temp_text=[]
            temp_text=words(get_text(doc_names))
...
def vocab(neg:Sequence[Sequence], pos:Sequence[Sequence]) -> dict:
    ...
    V = defaultintdict()
    V['__unknown__']=0
    complete_words= np.unique(np.concatenate(neg+pos))
    for i in range(0,len(complete_words)):
        V[complete_words[i]]=i+1
    return V
...
def vectorize_docs(docs:Sequence, V:dict) -> np.ndarray:
    ...
    D = np.zeros((n_docs, n_words ))
    for i in range(len(docs)):
        doc_vector = vectorize(V, docs[i])
        D[i] = doc_vector
...
class NaiveBayes621:
    ...
    def fit(self, X:np.ndarray, y:np.ndarray) -> None:
        ...
        P_X_neg = len(X[y == 0]) / total_docs
        P_X_pos = len(X[y == 1]) / total_docs
        words_in_neg = np.sum(X[y == 0], axis=0) 
        words_in_pos = np.sum(X[y == 1], axis=0)  
        log_k_neg = np.log((words_in_neg + 1) / (np.sum(words_in_neg) + X.shape[1]+1)) 
        log_k_pos = np.log((words_in_pos + 1) / (np.sum(words_in_pos) + X.shape[1]+1))
        
        self.log_P_X_neg = np.log(P_X_neg)
        self.log_P_X_pos = np.log(P_X_pos)
        self.log_k_neg = log_k_neg
        self.log_k_pos = log_k_pos
    ...
    def predict(self, X:np.ndarray) -> np.ndarray:
        ...
        for i in range(n_docs):
            log_ratio_neg = (self.log_P_X_neg + np.sum(X[i] * self.log_k_neg))
            log_ratio_pos = (self.log_P_X_pos + np.sum(X[i] * self.log_k_pos))
        
            predictions[i]=np.where(log_ratio_neg>log_ratio_pos,0,1)
        return predictions
